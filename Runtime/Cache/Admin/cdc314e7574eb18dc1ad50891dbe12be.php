...
<div class="table-operate ue-clear">
	<a href="/Thinkphp-1/index.php/Admin/Email/add" class="add">添加</a>
...
<div class="table-box">
	<table>
		...
		<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i;?><tr>
				...
				<td class="file"><?php echo ($vol["file"]); ?>
				<?php if($vol["hasfile"] == 1): ?>【<a href="javascript:;" class="download" data="<?php echo ($vol["id"]); ?>">下载附件</a>】<?php endif; ?>
				</td>
                ...
                <td class="status">
					<?php if($vol["isread"] == 0): ?><span style="color: red">未读</span>
						<?php else: ?>
						<span style="color:gray">已读</span><?php endif; ?>
				</td>
                <td class="operate">
                	<a href ='javascript:;' class="show" data="<?php echo ($vol["id"]); ?>" data_title="<?php echo ($vol["title"]); ?>" data_isread="<?php echo ($vol["isread"]); ?>">查看</a>
                </td>
...
<script type="text/javascript" src="/Thinkphp-1/Public/Admin/layer/layer.js"></script>
...
showRemind('input[type=text], textarea','placeholder');
	$(function(){
		$('.download').on('click',function(){
			...
			window.location.href='/Thinkphp-1/index.php/Admin/Email/download/id/'+id;
		});
		$('.show').on('click',function(){
			var id = $(this).attr('data');
			var title = $(this).attr('data_title');
			var isread = $(this).attr('data_isread');
			layer.open({
				type: 2,
				title: title,
				shadeClose: 0.8,
				area: ['380px', '90%'],
				content: '/Thinkphp-1/index.php/Admin/Email/getContent/id/'+id,
				end:function(){
					if(isread == 0){
